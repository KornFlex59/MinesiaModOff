...
public class AdminshopblockGravelScreen extends AbstractContainerScreen<AdminshopblockGravelMenu> {
	private final static HashMap<String, Object> guistate = AdminshopblockGravelMenu.guistate;
	...
	public AdminshopblockGravelScreen(AdminshopblockGravelMenu container, Inventory inventory, Component text) {
	...
	public void init() {
		super.init();
		number_buy = new EditBox(this.font, this.leftPos + 27, this.topPos + 88, 118, 18, Component.translatable("gui.palamod.adminshopblock_gravel.number_buy")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.palamod.adminshopblock_gravel.number_buy").getString());
				else
					setSuggestion(null);
			}
		...
		number_buy.setSuggestion(Component.translatable("gui.palamod.adminshopblock_gravel.number_buy").getString());
		guistate.put("text:number_buy", number_buy);
		this.addWidget(this.number_buy);
		button_buy = Button.builder(Component.translatable("gui.palamod.adminshopblock_gravel.button_buy"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockGravelButtonMessage(0, x, y, z));
				AdminshopblockGravelButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 26, this.topPos + 109, 40, 20).build();
		guistate.put("button:button_buy", button_buy);
		this.addRenderableWidget(button_buy);
		button_sell = Button.builder(Component.translatable("gui.palamod.adminshopblock_gravel.button_sell"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockGravelButtonMessage(1, x, y, z));
				AdminshopblockGravelButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 154, this.topPos + 5, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/cross_no_button.png"), ResourceLocation.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockGravelButtonMessage(2, x, y, z));
						AdminshopblockGravelButtonMessage.handleButtonAction(entity, 2, x, y, z);
					}
				}) {
			@Override
		...
		imagebutton_arrow_adminshop = new ImageButton(this.leftPos + 135, this.topPos + 5, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/arrow_adminshop.png"), ResourceLocation.parse("palamod:textures/screens/arrow_adminshop_poi.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockGravelButtonMessage(3, x, y, z));
						AdminshopblockGravelButtonMessage.handleButtonAction(entity, 3, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int x, int y, float partialTicks) {
		...
		imagebutton_home_pixel_adminshop = new ImageButton(this.leftPos + 118, this.topPos + 5, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/home_pixel_adminshop.png"), ResourceLocation.parse("palamod:textures/screens/pointec_home_pixel_adminshop.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockGravelButtonMessage(4, x, y, z));
						AdminshopblockGravelButtonMessage.handleButtonAction(entity, 4, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int x, int y, float partialTicks) {
