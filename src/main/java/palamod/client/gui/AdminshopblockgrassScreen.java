...
public class AdminshopblockgrassScreen extends AbstractContainerScreen<AdminshopblockgrassMenu> {
	...
	public void init() {
		super.init();
		number_buy = new EditBox(this.font, this.leftPos + 27, this.topPos + 88, 118, 18, Component.translatable("gui.palamod.adminshopblockgrass.number_buy")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.palamod.adminshopblockgrass.number_buy").getString());
				else
					setSuggestion(null);
			}
		...
		number_buy.setSuggestion(Component.translatable("gui.palamod.adminshopblockgrass.number_buy").getString());
		guistate.put("text:number_buy", number_buy);
		this.addWidget(this.number_buy);
		button_buy = Button.builder(Component.translatable("gui.palamod.adminshopblockgrass.button_buy"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockgrassButtonMessage(0, x, y, z));
		...
		button_sell = Button.builder(Component.translatable("gui.palamod.adminshopblockgrass.button_sell"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockgrassButtonMessage(1, x, y, z));
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 154, this.topPos + 4, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/cross_no_button.png"), ResourceLocation.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockgrassButtonMessage(2, x, y, z));
