...
import palamod.network.AdminshopblockoakwoodButtonMessage;
...
public class AdminshopblockoakwoodScreen extends AbstractContainerScreen<AdminshopblockoakwoodMenu> {
	private final static HashMap<String, Object> guistate = AdminshopblockoakwoodMenu.guistate;
	...
	public AdminshopblockoakwoodScreen(AdminshopblockoakwoodMenu container, Inventory inventory, Component text) {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.adminshopblockoakwood.label_oak_wood"), 58, 6, -1, false);
		guiGraphics.drawString(this.font,

				AdshoppreviewamountoakwoodProcedure.execute(world, entity, guistate), 29, 74, -4671036, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.adminshopblockoakwood.label_sell_price_05"), 39, 30, -4671036, false);
		guiGraphics.drawString(this.font, Component.translatable("gui.palamod.adminshopblockoakwood.label_buy_price_10"), 41, 43, -4671036, false);
	}

	@Override
	public void init() {
		super.init();
		number_buy = new EditBox(this.font, this.leftPos + 27, this.topPos + 88, 118, 18, Component.translatable("gui.palamod.adminshopblockoakwood.number_buy")) {
			@Override
			public void insertText(String text) {
				super.insertText(text);
				if (getValue().isEmpty())
					setSuggestion(Component.translatable("gui.palamod.adminshopblockoakwood.number_buy").getString());
				else
					setSuggestion(null);
			}
		...
		number_buy.setSuggestion(Component.translatable("gui.palamod.adminshopblockoakwood.number_buy").getString());
		guistate.put("text:number_buy", number_buy);
		this.addWidget(this.number_buy);
		button_buy = Button.builder(Component.translatable("gui.palamod.adminshopblockoakwood.button_buy"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockoakwoodButtonMessage(0, x, y, z));
				AdminshopblockoakwoodButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 26, this.topPos + 109, 40, 20).build();
		guistate.put("button:button_buy", button_buy);
		this.addRenderableWidget(button_buy);
		button_sell = Button.builder(Component.translatable("gui.palamod.adminshopblockoakwood.button_sell"), e -> {
			if (true) {
				PacketDistributor.sendToServer(new AdminshopblockoakwoodButtonMessage(1, x, y, z));
				AdminshopblockoakwoodButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		...
		imagebutton_cross_no_button = new ImageButton(this.leftPos + 153, this.topPos + 4, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/cross_no_button.png"), ResourceLocation.parse("palamod:textures/screens/pointed_cross_no_button.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockoakwoodButtonMessage(2, x, y, z));
						AdminshopblockoakwoodButtonMessage.handleButtonAction(entity, 2, x, y, z);
					}
				}) {
			@Override
		...
		imagebutton_arrow_adminshop = new ImageButton(this.leftPos + 134, this.topPos + 4, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/arrow_adminshop.png"), ResourceLocation.parse("palamod:textures/screens/arrow_adminshop_poi.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockoakwoodButtonMessage(3, x, y, z));
						AdminshopblockoakwoodButtonMessage.handleButtonAction(entity, 3, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int x, int y, float partialTicks) {
		...
		imagebutton_home_pixel_adminshop = new ImageButton(this.leftPos + 118, this.topPos + 4, 16, 16,
				new WidgetSprites(ResourceLocation.parse("palamod:textures/screens/home_pixel_adminshop.png"), ResourceLocation.parse("palamod:textures/screens/pointec_home_pixel_adminshop.png")), e -> {
					if (true) {
						PacketDistributor.sendToServer(new AdminshopblockoakwoodButtonMessage(4, x, y, z));
						AdminshopblockoakwoodButtonMessage.handleButtonAction(entity, 4, x, y, z);
					}
				}) {
			@Override
			public void renderWidget(GuiGraphics guiGraphics, int x, int y, float partialTicks) {
