...
public class PalamodModTrades {
	...
	public static void registerTrades(VillagerTradesEvent event) {
		if (event.getType() == PalamodModVillagerProfessions.SANTA.get()) {
			...
			event.getTrades().get(3).add(new BasicItemListing(new ItemStack(Blocks.AMETHYST_BLOCK, 10), new ItemStack(Blocks.JUKEBOX), 10, 5, 0.05f));
			event.getTrades().get(5).add(new BasicItemListing(new ItemStack(PalamodModBlocks.PALADIUM_GREEN_BLOCK.get(), 2), new ItemStack(PalamodModItems.ENDIUM_NUGGET.get(), 2), 1, 5000, 0f));
			event.getTrades().get(4).add(new BasicItemListing(new ItemStack(PalamodModBlocks.PALADIUM_GREEN_BLOCK.get(), 4), new ItemStack(Blocks.DIRT, 2), 10, 1500, 0.05f));
		}
	}
}
