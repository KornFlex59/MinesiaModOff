...
public record AdminshopmobswitherskullButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = AdminshopmobswitherskullMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			AdhmobsbuywitherskullProcedure.execute(world, entity, guistate);
		}
		if (buttonID == 1) {

			AdhmobssellwitherskullProcedure.execute(world, entity, guistate);
		}
		if (buttonID == 2) {
