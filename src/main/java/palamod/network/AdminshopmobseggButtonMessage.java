...
public record AdminshopmobseggButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = AdminshopmobseggMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			AdhmobsbuyeggProcedure.execute(world, entity, guistate);
		}
		if (buttonID == 1) {

			AdhmobsselleggProcedure.execute(world, entity, guistate);
		}
		if (buttonID == 2) {
